...
- `notify` — student/employee 角色可用，写入通知数据（通知类型：leave_request/leave_result/complaint/complaint_result/exam_reminder/psychology_warning/progress_update/todo_reminder）

## Skills 动态加载

- `skills/` 目录由 `SkillLoader` 在运行时动态扫描 `SKILL.md` 文件
- Agent 调用 `load_skill` 工具加载（返回 `(no skills)` 若目录为空）
...
JWT_SECRET_KEY     # JWT签名密钥（生产必须改，代码中有默认值 dev-secret-key-...）
EMBEDDINGS_API_KEY # 向量嵌入密钥
EMBEDDINGS_BASE_URL # 向量嵌入端点
EMBEDDINGS_MODEL   # 向量嵌入模型，如 text-embedding-v3
...
- `generate_report` 工具对应 `app/reports/generators/` 下的报告生成器（仅 employee 角色）