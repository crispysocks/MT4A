...
- `app/db/connection.py` 的 `init_db_on_startup()` 启动时自动建表

## 前端资源

- 静态文件在 `app/api/static/`（index.html, login.html, app.js）
- 登录页：`/login`；聊天页：`/`
- 前端**无状态**，会话由后端 `SessionManager` 管理（`.conversations/*.json`）

## 测试

- 测试文件在 `tests/`（被 .gitignore 忽略，不会 push）
- 重要 fixtures：`tests/conftest.py` 提供 SQLite 内存数据库 fixture
- 测试 auth/soul/knowledge_router 需要正确 import 已注册的工具
